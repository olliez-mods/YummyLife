...
latest `OneLife_v*` / `AnotherPlanet_v*` tag. To run the script by hand, put the data
repos there yourself first. An OHOL folder and an AHAP folder differ only by their data
and by an `isAHAP.txt` file, which `game.cpp` reads at startup.

The release notes, including the download tables, are generated by
`yummyLifeBuildScripts/makeReleaseNotes.sh` from the assets actually built.
